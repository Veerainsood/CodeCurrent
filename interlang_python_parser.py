...
class CallVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name):
            self.function_calls[node.func.id].append((node.lineno, node.args))
    ...
    def _get_annotation(self, annotation_node):
    ...
    def _report(self, language, callee, args, call_node):
        if self.func_stack:
            ...
            caller_types = [self._get_annotation(arg.annotation) for arg in caller_node.args.args]
            caller_args_count = len(caller_args)
            caller_param_set = set(caller_args)

            actuals = []
            for lineno, arg_nodes in self.function_calls.get(caller_function, []):
                actuals = [self._get_type(arg) for arg in arg_nodes]
                break
        else:
        ...
        param_usages = []
        for arg in args:
            if isinstance(arg, ast.Name) and arg.id in caller_param_set:
                param_usages.append(arg.id)
